...
from impomarit.models import VEmbarqueaereo, Cargaaerea, Envases, Serviceaereo
...
def get_data_email_op(request):
    ...
    if is_ajax(request):
        try:
            ...
            texto += f'<br>'
            texto, resultado = get_data_html(row_number, row, row2, title, texto, resultado)
...
def get_data_html(row_number, row, row2, title, texto, resultado):
    if title == 'Notificación de transbordo de carga':
        ...
        campos = [
            ...
            ("LLegada estimada: ", str(row.fecha_retiro) if row.fecha_retiro is not None else "S/I"),
            ...
            ("Referencia: ", str(row.referencia) if row.referencia is not None else "S/I"),
            ...
            ("Nro. Contenedor: ", str(row2.nrocontenedor) if row2.nrocontenedor is not None else "S/I"),
            ("Bultos: ", str(row2.bultos) if row2.bultos is not None else "S/I"),
            ("Peso: ", str(row2.bruto) if row2.bruto is not None else "S/I"),
            ("Mercadería: ", str(row2.mercaderia) if row2.mercaderia is not None else "S/I")
        ]
                ...
                <table border= "1" style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style="padding: 8px;">{str(row.fecha_embarque) if row.fecha_embarque is not None else "S/I"}</td>
                            <td style="padding: 8px;">{str(row.fecha_retiro) if row.fecha_retiro is not None else "S/I"}</td>
                        </tr>
                    </tbody>
                </table>
                <br>
        ...
        precinto = Envases.objects.get(numero=row_number)
        fecha_actual = datetime.datetime.now()
        # ASUNTO DEL MENSAJE
        ...
        tabla_html = "<table border= '1' style='width: 40%; border-collapse: collapse;'>"
        campos = [
            ("Embarque: ", str(row.referencia) if row.referencia is not None else "S/I"),
            ("Posición: ", str(row.posicion) if row.posicion is not None else "S/I"),
            ("Salida: ", str(row.fecha_embarque) if row.fecha_embarque is not None else "S/I"),
            ("LLegada: ", str(row.fecha_retiro) if row.fecha_retiro is not None else "S/I"),
            ("Origen: ", str(row.origen) if row.origen is not None else "S/I"),
            ...
            ("Bultos: ", str(row2.bultos) if row2.bultos is not None else "S/I"),
            ("Peso: ", str(row2.bruto) if row2.bruto is not None else "S/I"),
            ("CBM: ", str(row2.cbm) if row2.cbm is not None else "S/I"),
            ("Nro. Contenedor: ", str(row2.nrocontenedor) if row2.nrocontenedor is not None else "S/I"),
            ("Precintos: ", str(precinto) if precinto is not None else "S/I"),
        ]
        ...
        # row3 = Serviceaereo.objects.get(numero=row_number)
        ...
        campos1 = [
            ("Fecha: ", str(row.fecha_embarque) if row.fecha_embarque is not None else "S/I"),
        ...
        campos2 = [
            ("Proveedor: ", str(row.transportista) if row.transportista is not None else "S/I"),
            ("Direccion: ", str(row.direccion_transportista) if row.direccion_transportista is not None else "S/I"),
            ("Ciudad: ", str(row.ciudad_transportista) if row.ciudad_transportista is not None else "S/I"),
            ("Pais: ", str(row.pais_transportista) if row.pais_transportista is not None else "S/I"),
        ]
        ...
        campos4 = [
            ("Referencia interna: ", str(row.referencia) if row.referencia is not None else "S/I"),
            ("Orden cliente: ", str(row.orden_cliente) if row.orden_cliente is not None else "S/I"),
            ("Origen: ", str(row.origen) if row.origen is not None else "S/I"),
            ("Destino: ", str(row.destino) if row.destino is not None else "S/I"),
        ]
        ...
        campos5 = [
            ...
            ("Bultos: ", str(row2.bultos) if row2.bultos is not None else "S/I"),
            ("Peso: ", str(row2.bruto) if row2.bruto is not None else "S/I"),
            # ("Condiciones de pago: ", str(row3.modo) if row3.modo is not None else "S/I"),
