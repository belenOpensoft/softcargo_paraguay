...
from expaerea.forms import add_im_form, add_form, add_house, edit_form, edit_house, gastosForm, gastosFormHouse, \
    rutasFormHouse, emailsForm, embarquesFormHouse, NotasForm, rutasFormMaster, edit_house_general
from expaerea.models import Master, ExportEmbarqueaereo, VEmbarqueaereo, VEmbarqueaereoDirecto, ExportAttachhijo, \
    ExportCargaaerea, \
    ExportServiceaereo, ExportConexaerea, ExportFaxes, ExportReservas
...
@login_required(login_url='/')
def embarques_exportacion_aerea(request):
    try:

        if request.user.has_perms(["expaerea.view_master", ]):

            return render(request, 'expaerea/grilla_datos_general.html', {
                'form': add_form(),
                'form_house': add_house(),
                'form_search_master': add_im_form(),
                'form_edit_master': edit_form(),
...
def source_embarques_general(request):
    if is_ajax(request):
        ...
        reservas = ExportReservas.objects.all().values_list('awb', 'numero')
    ...
    try:
        ...
        for registro in registros_filtrados:
            ...
            if id_reserva:
                ...
                registro_json.append('DIRECTO') #18
