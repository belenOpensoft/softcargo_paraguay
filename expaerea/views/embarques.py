...
def source_embarques(request):
    if is_ajax(request):
        ...
        registros = ExportCargaaerea.objects.filter(numero=numero).order_by(*order)
        ...
        resultado['recordsTotal'] = ExportCargaaerea.objects.filter(numero=numero).count()
...
def get_data(registros_filtrados):
    try:
        ...
        for registro in registros_filtrados:
            ...
            registro_json.append('' if registro.aplicable is None else str(registro.aplicable))
            registro_json.append('' if registro.tarifa is None else str(registro.tarifa))
...
def guardar_embarques(request):
    ...
    try:
        ...
        if 'id_embarque' in data and data['id_embarque']:
            ...
            try:
                registro = ExportCargaaerea.objects.get(id=data['id_embarque'])
            except ExportCargaaerea.DoesNotExist:
    ...
    try:
        ...
        if isinstance(data, list):
            for envase_data in data:
                # Crear el registro del modelo ExportCargaaerea
                registro = ExportCargaaerea()

                # Obtener los campos disponibles del modelo
                campos = [f.name for f in ExportCargaaerea._meta.fields]
...
def eliminar_embarque(request):
    ...
    try:
        id = request.POST['id']
        ExportCargaaerea.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
