...
$(document).ready(function () {
    ...
    table_general_ea = $('#tabla_general_ea').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "columnDefs": [
            {
                ...
                render: function (data, type, row) {
                    return `<span class="badge bg-primary text-light">${row[18] ?? ''}</span>`;
                }
            },
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/exportacion_aerea/source_embarques_general",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
        ...
        initComplete: function () {
            ...
            let state = table_general_ea.state.loaded();
            if (state) {
    ...
    table_general_ea.columns().iterator('column', function (ctx, idx) {
        table_general_ea.column(idx).nodes().to$().addClass('columna-acciones');
    });

    table_general_ea.on('draw', function () {
        // Evento tras redibujado
    });

    $(table_general_ea.table().container()).on('keyup', 'tfoot input', function () {
        $(this).addClass('is-invalid');
        table_general_ea
    ...
    $('#tabla_general_ea tfoot th').each(function(index) {
        let title = $('#tabla_general_ea thead th').eq(index).text();
    ...
    $('#tabla_general_ea tbody').on('click', 'tr', function (event) {
        event.stopPropagation();

        if (!$(this).hasClass('table-secondary')) {
            $('#tabla_general_ea tbody tr').removeClass('table-secondary');
            $(this).addClass('table-secondary');
        }
    });

    $('#tabla_general_ea tbody').on('dblclick', 'tr', function() {
        var tr = $(this).closest('tr');
        var row = table_general_ea.row(tr);
        ...
        if (rowData) {
            ...
            localStorage.setItem('tabla_origen', 'table_general_ea');
    ...
    $('#edit_house_form_general').submit(function(e){
        ...
        $('#edit_house_form_general').attr('action', '/exportacion_aerea/edit_house/' + numero + '/');
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    table_general_ea.ajax.reload(null, false);
...
function fillFormWithDataHouseGeneral(data) {
    ...
    $('#id_pago_general').val(data.pagoflete_e);     // pago (general)
...
function filtrar_tabla_general(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/exportacion_aerea/buscar_registros_general/',
