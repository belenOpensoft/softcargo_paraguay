...
$(document).ready(function () {
    ...
    $('#buscadorEmailsHouse12').on('keyup', function () {
        let valor = $(this).val().toLowerCase();
        $("#listaEmailsHouse12 tr").filter(function () {
    ...
    /* DATATABLES */
    //buscadores
$('#tabla_expoterrestre tfoot th').each(function(index) {
    let title = $('#tabla_expoterrestre thead th').eq(index).text();
    ...
    table = $('#tabla_expoterrestre').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/exportacion_terrestre/source_master",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "buscar": buscar,
...
//        },
    ...
    $("#destino_add").autocomplete({
        ...
        select: function (event, ui) {
            $(this).attr('data-id', ui.item['id']);
            $('#discharge_add').val(ui.item['value']);
...
//        }
    ...
    $.ajax({
        type: "POST",
        url: "/exportacion_terrestre/add_master/",
        data: formData,
        ...
        //ver mas
var expandedRow;
    $('#tabla_expoterrestre tbody').on('click', 'td.details-control', function () {
        ...
        tr.removeClass('shown');
            ...
            $.ajax({
                url: '/exportacion_terrestre/source_embarque_aereo/',
                type: 'POST',
                data: {
     ...
     //modificar master
    $('#tabla_expoterrestre tbody').on('click', 'td', function () {
                    ...
                    buttons: [
                        {
                            ...
                            validarCoincidenciaAcumulados();
                                ...
                                $('#table_edit_et').DataTable().destroy();
    ...
    $('#tabla_expoterrestre tbody').on('dblclick', 'tr', function() {
        var tr = $(this).closest('tr');
        var row = table.row(tr);
        var rowData = row.data();
    ...
    alert('No se admiten valores negativos.');
        ...
        $('#edit_master_form').attr('action', '/exportacion_terrestre/edit_master/' + id_master + '/');
...
//                    $('#table_edit_et').DataTable().destroy();
   ...
   let vector = JSON.parse(localStorage.getItem('fecha_editada_house')) || [];
    ...
    // tabla general
    if (!$(event.target).closest('#tabla_expoterrestre').length) {
        $('#tabla_expoterrestre tbody tr').removeClass('table-secondary');
    }
//    //tabla de houses en edit master
//    if (!$(event.target).closest('#table_edit_et').length) {
//        $('#table_edit_et tbody tr').removeClass('table-secondary');
//    }
//    //tabla de houses en add master
//    if (!$(event.target).closest('#table_add_et').length) {
//        $('#table_add_et tbody tr').removeClass('table-secondary');
//    }
});
    $('#tabla_expoterrestre tbody').on('click', 'tr', function (event) {
   ...
   //form add house
                    ...
                    buttons: [
                        {
                           ...
                           click: function () {
                               $(this).dialog("close");
                               if (table_add_et instanceof $.fn.dataTable.Api) {
                               $('#table_add_et').DataTable().destroy();
                               $('#segment_response').css({'display':'none'});
                               }
                           },
                       },
    ...
    $.ajax({
        type: "POST",
        url: "/exportacion_terrestre/add_house/",
        data: formData,
        success: function(response) {
            if (response.success) {
            if(lugar_editar==='agregar'){
            if (table_add_et instanceof $.fn.dataTable.Api) {
                    table_add_et.ajax.reload(null, false);
                } else {
                    cargar_hauses_master();
                }
            }else if(lugar_editar==='editar'){
            if (table_edit_et instanceof $.fn.dataTable.Api) {
                    table_edit_et.ajax.reload(null, false);
                } else {
                    cargar_hauses_master_edit();
                }
            }else if(lugar_editar==='edit_directo'){
                $('#tabla_house_directo_et').DataTable().ajax.reload(null, false);
                }
   ...
   //modificar house
    ...
    alert('No se admiten valores negativos en los campos numéricos.')
        ...
        var formData = $(this).serialize();
        $('#edit_house_form').attr('action', '/exportacion_terrestre/edit_house/' + numero + '/');
                        ...
                        buttons: [
                            {
                                ...
                                click: function () {
                                    if (confirm('¿Confirma eliminar el gasto seleccionado?')) {
                                        ...
                                        if (row.length === 1) {
                                            miurl = "/exportacion_terrestre/eliminar_gasto_master/";
                                            var toData = {
                                                'id': row[0][0],
    ...
    if (confirm("¿Confirma guardar el gasto?")) {
        ...
        let numero=localStorage.getItem('numero_master_seleccionado');
            ...
            alert(numero);
            miurl = "/exportacion_terrestre/add_gasto_master/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    $('#tabla_gastos tbody').on('dblclick', 'tr', function () {
        ...
        $("#id_pinformar").val(data[10]);
        $("#id_servicio").val(data[13]);
        $("#id_moneda_id").val(data[14]);
        $("#id_socio").val(data[15]);
    ...
    if (confirm("¿Confirma guardar el gasto?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/exportacion_terrestre/add_gasto_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
                ...
                success: function (resultado) {
                    if (resultado['resultado'] === 'exito') {
                       ...
                       document.getElementById('numero_gasto_house').value=aux;
                        if ($.fn.DataTable.isDataTable('#table_add_et')) {
                            $('#table_add_et').DataTable().ajax.reload(null, false);
                        }

                        if ($.fn.DataTable.isDataTable('#table_edit_et')) {
                            $('#table_edit_et').DataTable().ajax.reload(null, false);
                        }
                        if ($.fn.DataTable.isDataTable('#tabla_house_directo_et')) {
                            $('#tabla_house_directo_et').DataTable().ajax.reload(null, false);
                        }
                    } else {
                        alert(resultado['resultado']);
    ...
    if (confirm("¿Confirma guardar la ruta?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/exportacion_terrestre/add_ruta_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    $('#tabla_rutas_house tbody').off('dblclick').on('dblclick', 'tr', function () {
        ...
        $("#id_salida").val(data[3]);
        $("#id_llegada").val(data[4]);
        $("#id_viaje_ruta").val(data[5]);
        $("#id_modo_ruta").val(data[7]);
        $("#id_cia").val(data[6]);
    ...
    if (confirm("¿Confirma guardar el envase?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/exportacion_terrestre/add_envase_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    if (confirm("¿Confirma guardar el envase?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/exportacion_terrestre/add_embarque_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
                    ...
                    },
                {
                    ...
                    click: function () {
                        if (confirm('¿Confirma descargar el archivo seleccionado?')) {
                            var url = '/exportacion_terrestre/descargar_archivo/' + localStorage.getItem('id_archivo');  // Ruta de la vista que devuelve el archivo
                    ...
                    click: function () {
                        if (confirm('¿Confirma eliminar archivo?')) {
                            if (localStorage.getItem('id_archivo')) {
                                miurl = "/exportacion_terrestre/eliminar_archivo/";
                                var toData = {
    ...
    $('#guardar_archivo').click(function () {
        if (confirm("¿Confirma guardar archivo?")) {
            ...
            miurl = "/exportacion_terrestre/guardar_archivo/";
            $.ajax({
                type: "POST",
                url: miurl,
                data: formData,
...
table_add_et = $('#table_add_et').DataTable({
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_add_et th').css({'width':'auto'});
         $('#table_add_et_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    $('#table_add_et tbody').off('dblclick').on('dblclick', 'tr', function () {
        var tr = $(this).closest('tr');
        var row = table_add_et.row(tr);
        var rowData = row.data();
        ...
        if (rowData) {
            ...
            $.ajax({
            url: '/exportacion_terrestre/house-detail',
            data: { id: selectedRowId},
            method: 'GET',
            success: function (data) {
    ...
    $('#table_add_et tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_add_et tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('tabla_origen', 'table_add_et');
        localStorage.setItem('clase_house', 'ET');
...
function fillFormWithDataHouse(data) {
        ...
        $('#terminos_he').val(data.terminos_e);
        $('#id_tipo_e').val(data.tipo_e);
...
function getNameByIdVendedor(id) {
    var name = "";
    $.ajax({
        url: '/exportacion_terrestre/get_name_by_id_vendedor',
        data: { id: id},
        async: false,
        success: function (response) {
            name = response.name;
...
function fillFormWithData(data) {
    ...
    $('#edit_master_form [name="volumen_e"]').val(data.volumen_e);
...
var name = "";
$.ajax({
    url: '/exportacion_terrestre/get_name_by_id',
    data: { id: id},
    async: false,
    success: function (response) {
        name = response.name;
...
table_edit_et = $('#table_edit_et').DataTable({
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_edit_et th').css({'width':'auto'});
         $('#table_edit_et_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    $('#table_edit_et tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_edit_et tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('id_house_gasto', selectedRowId);
        localStorage.setItem('num_house_gasto', selectedRowN);
        localStorage.setItem('tabla_origen', 'table_edit_et');
                    localStorage.setItem('clase_house', 'ET');
...
function traer_seguimientos() {
    ...
    $.ajax({
        url: '/exportacion_terrestre/source_seguimientos_importado/',
        type: 'POST',
        dataType: 'json',
        ...
        success: function(response) {
            ...
            if(localStorage.getItem('lugar_importarhijo')==='editmaster'){
             ...
             $.ajax({
                type: "GET",
                url: "/exportacion_terrestre/generar_posicion/",
...
function guardar_importado_house(data, seguimientos) {
    $.ajax({
        url: '/exportacion_terrestre/add_house_importado/',
        type: 'POST',
...
function traer_gastos_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_terrestre/source_gastos_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
...
function guardar_gasto_importado(data){
    miurl = "/exportacion_terrestre/add_gasto_importado/";
...
function traer_envases_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_terrestre/source_envases_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_envase_importado(data){
    miurl = "/exportacion_terrestre/add_envase_importado/";
...
function traer_rutas_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_terrestre/source_rutas_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_ruta_importado(data){
    miurl = "/exportacion_terrestre/add_ruta_importado/";
...
function traer_embarques_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_terrestre/source_embarque_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_embarque_importado(data){
    miurl = "/exportacion_terrestre/add_embarque_importado/";
...
function traer_archivos_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_terrestre/source_archivos_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_archivo_importado(data) {
    const miurl = "/exportacion_terrestre/add_archivo_importado/";
                ...
                success: function (data) {
                    ...
                    if (localStorage.getItem('lugar_importarhijo') === 'editmaster') {
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_edit_et.rows('.table-secondary').data();
                            if (row.length === 1) {
                                let master = localStorage.getItem('master_editar');
                        ...
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_add_et.rows('.table-secondary').data();
                            if (row.length === 1) {
                                miurl = "/exportacion_terrestre/eliminar_house/";
                                var toData = {
                                    'id': row[0][0],
...
function eliminar_agregado(master,id_house){
        ...
        $.ajax({
        type: "POST",
        url: "/exportacion_terrestre/source_embarque_id/",
        data: {
            id: id_house,
        ...
        success: function(response) {
            $.ajax({
                type: "POST",
                url: "/exportacion_terrestre/source_seguimiento_id/",
                data: {
...
let numero=localStorage.getItem('numero_master_seleccionado');
    ...
    table_gastos = $('#tabla_gastos').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_gastos/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
function get_datos_gastos_house() {
    ...
    table_gastos = $('#tabla_gastos_house').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_gastos_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
         ...
         url='master-detail/';
            ...
            url: '/exportacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    if ($.fn.dataTable.isDataTable('#table_edit_et')) {
                        consignatario_code = $('#table_edit_et').DataTable().row('.table-secondary').data()[20];
                    } else {
                        consignatario_code = $('#tabla_house_directo_et').DataTable().row('.table-secondary').data()[20];
                    }
                    if (selectedRowN != null) {
                        get_datos_gastos_house();
                        $('#gastos_form_house').trigger("reset");
                            ...
                            buttons: [
                                {
                                    ...
                                    click: function () {
                                        if (confirm('¿Confirma eliminar el gasto seleccionado?')) {
                                            row = table_gastos.rows('.table-secondary').data();
                                            if (row.length === 1) {
                                                miurl = "/exportacion_terrestre/eliminar_gasto_house/";
                                                var toData = {
                                                    'id': row[0][0],
                                                    'csrfmiddlewaretoken': csrf_token,
                                                };
...
//                $('#table_add_et tbody tr').removeClass('table-secondary');
//                $('#table_edit_et tbody tr').removeClass('table-secondary');
//                $('#tabla_house_directo_et tbody tr').removeClass('table-secondary');
                                // table.ajax.reload();
                                $("#tabla_gastos").dataTable().fnDestroy();
                            }
                        })
...
function get_datos_rutas_house() {
   ...
   $("#tabla_rutas_house").dataTable().fnDestroy();
    table_rutas = $('#tabla_rutas_house').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_rutas_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
                            ...
                            buttons: [
                                {
                                    ...
                                    click: function () {
                                        if (confirm('¿Confirma eliminar el gasto seleccionado?')) {
                                            ...
                                            if (row.length === 1) {
                                                miurl = "/exportacion_terrestre/eliminar_ruta_house/";
                                                var toData = {
                                                    'id': row[0][0],
                                                    'csrfmiddlewaretoken': csrf_token,
                                                };
...
function get_datos_seguimiento_rutas(numero) {
    $.ajax({
        url: "/exportacion_terrestre/datos_embarque_ruta/",  // Asegúrate de que esta URL coincida con tu Django URLConf
        type: "POST",
        data: {
            numero: numero,
...
function get_datos_envases_house(){
   ...
   $("#tabla_envases_house").dataTable().fnDestroy();
    table_envases = $('#tabla_envases_house').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_envases_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
                            ...
                            buttons: [
                                {
                                    ...
                                    click: function () {
                                        if (confirm('¿Confirma eliminar el seleccionado?')) {
                                            ...
                                            if (row.length === 1) {
                                                miurl = "/exportacion_terrestre/eliminar_envases_house/";
                                                var toData = {
                                                    'id': row[0][0],
                                                    'csrfmiddlewaretoken': csrf_token,
                                                };
...
function get_sugerencias_envases(numero) {
    ...
    $.ajax({
        type: "GET",
        url: "/exportacion_terrestre/get_sugerencias_envases/" + numero,
        success: function(response) {
        console.log(response.data);
 ...
 let numero=localStorage.getItem('num_house_gasto');
   ...
   $("#tabla_embarques_house").dataTable().fnDestroy();
    table_embarques = $('#tabla_embarques_house').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_embarques_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
        ...
        "columnDefs": [
        {
            "targets": 8,  // Índice de la columna a ocultar (empieza en 0)
            "visible": false,
                            ...
                            buttons: [
                                {
                                    ...
                                    click: function () {
                                        if (confirm('¿Confirma eliminar el embarque seleccionado?')) {
                                            ...
                                            if (row.length === 1) {
                                                miurl = "/exportacion_terrestre/eliminar_embarques_house/";
                                                var toData = {
                                                    'id': row[0][0],
                                                    'csrfmiddlewaretoken': csrf_token,
                                                };
...
$('.email').click(function () {
    $("#modalSeleccionEmailHouse12").dialog('close');
        ...
        var row = $('#table_edit_et').DataTable().rows('.table-secondary').data();
        $("#id_to").val('');
        $("#id_cc").val('');
        $("#id_cco").val('');
        cco = $("#id_subject").val('');
...
function get_data_email(row,title,numero,id,master,gastos) {
    let miurl = "/exportacion_terrestre/get_data_email/";
    var toData = {
        'title': title,
        'row_number': numero,
        'id':id,
...
function get_datos_archivos() {
   let table_archivos = $('#tabla_archivos').DataTable({
        ...
        "ajax": {
            "url": "/exportacion_terrestre/source_archivos/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
...
let lugar=localStorage.getItem('lugar');
    ...
    $.ajax({
        type: "POST",
        url: "/exportacion_terrestre/modificar_fecha_retiro/",
...
function get_datos_pdf() {
    let selectedRowN = localStorage.getItem('num_house_gasto');
    miurl = "/exportacion_terrestre/get_datos_caratula/";
    var toData = {
        'numero': selectedRowN,
        'csrfmiddlewaretoken': csrf_token,
    };
...
function acumulados(master, callback) {
    ...
    $.ajax({
        url: '/exportacion_terrestre/source_embarque_aereo_full/' + master + '/',
        method: 'GET',
        success: function(response) {
...
function cargar_notas(numero) {
    $('#notas_table').DataTable({
        ...
        ajax: {
            url: `/exportacion_terrestre/source/?numero=${numero}`,  // URL de la vista source
            dataSrc: 'data'
        },
        columns: [
...
function agregar_nota(event) {
    ...
    const url = "/exportacion_terrestre/guardar_notas/";
...
function eliminarNota(id) {
    if (confirm("¿Desea eliminar esta nota?")) {
        $.ajax({
            type: "POST",
            url: `/exportacion_terrestre/eliminar_nota/`,
            data: {
            ...
            url: '/exportacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    let tabla_factura = $('#facturar_table').DataTable({
                        ...
                        "ajax": {
                            "url": "/exportacion_terrestre/source_gastos_house_preventa/",
                            'type': 'GET',
                            "data": function (d) {
                                return $.extend({}, d, {
                                    "numero": numero,
         ...
         url='master-detail/';
            ...
            url: '/exportacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    row = table_edit_et.rows('.table-secondary').data();
                    console.log(row);
                    if (row.length === 1) {
                        $("#logs_modal").dialog({
                            autoOpen: true,
                        ...
                        table_logs = $('#tabla_logs').DataTable({
                            ...
                            "ajax": {
                                "url": "/exportacion_terrestre/source_logs/",
                                'type': 'GET',
                                "data": function (d) {
                                    return $.extend({}, d, {
