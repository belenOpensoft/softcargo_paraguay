...
$(document).ready(function () {
    ...
    /* DATATABLES */
    //buscadores
    $('#tabla_exportaerea tfoot th').each(function () {
        let title = $(this).text();
        if (title !== '') {
...
function aplicable_volumen(volumen){
    ...
    $('#tabla_exportaerea tbody').on('click', 'td.details-control', function () {
        ...
        tr.removeClass('shown');
            ...
            $.ajax({
                url: '/exportacion_aerea/source_embarque_aereo/' + selectedRowId + '/',
                type: 'GET',
                success: function (response) {
               // console.log(data);
...
table_add_im = $('#table_add_im').DataTable({
   ...
   "columnDefs": [
    ...
    "pageLength": 100,
    "ajax": {
        "url": `/exportacion_aerea/source_embarque_aereo/${master}/`,
        "type": 'POST',
        "headers": {
...
function gastos_btn_h_click(){
        ...
        if ($.fn.dataTable.isDataTable('#table_edit_im')) {
            consignatario_code = $('#table_edit_im').DataTable().row('.table-secondary').data()[18];
        } else {
