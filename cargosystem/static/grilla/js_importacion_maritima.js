...
var expandedRow;
    $('#tabla_importmarit tbody').on('click', 'td.details-control', function () {
        ...
        tr.removeClass('shown');
            ...
            $.ajax({
                url: '/importacion_maritima/source_embarque_aereo/',
                type: 'POST',
                data: {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (table_edit_im instanceof $.fn.dataTable.Api) {
                ...
                $('#tabla_house_directo_im').DataTable().ajax.reload(null, false);
                }
   ...
   //modificar house
                ...
                success: function (resultado) {
                    if (resultado['resultado'] === 'exito') {
                       ...
                       document.getElementById('numero_gasto_house').value=aux;
                        ...
                        if ($.fn.DataTable.isDataTable('#tabla_house_directo_im')) {
                            $('#tabla_house_directo_im').DataTable().ajax.reload(null, false);
                        }
                    } else {
                        alert(resultado['resultado']);
    ...
    if (confirm("¿Confirma guardar el Embarque?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/importacion_maritima/add_embarque_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    $('#guardar_archivo').click(function () {
        if (confirm("¿Confirma guardar archivo?")) {
            ...
            miurl = "/importacion_maritima/guardar_archivo/";
            $.ajax({
                type: "POST",
                url: miurl,
                data: formData,
...
table_add_im = $('#table_add_im').DataTable({
    ...
    if (rowData) {
        ...
        localStorage.setItem('tabla_origen', 'table_add_im');
...
function getNameByIdVendedor(id) {
    var name = "";
    $.ajax({
        url: '/importacion_maritima/get_name_by_id_vendedor',
        data: { id: id},
        async: false,
        success: function (response) {
            name = response.name;
...
table_edit_im = $('#table_edit_im').DataTable({
    ...
    if (rowData) {
        ...
        localStorage.setItem('id_house_gasto', selectedRowId);
        localStorage.setItem('num_house_gasto', selectedRowN);
        localStorage.setItem('tabla_origen', 'table_edit_im');
...
function traer_gastos_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_maritima/source_gastos_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
...
function guardar_gasto_importado(data){
    miurl = "/importacion_maritima/add_gasto_importado/";
...
function traer_envases_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_maritima/source_envases_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_envase_importado(data){
    miurl = "/importacion_maritima/add_envase_importado/";
...
function traer_rutas_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_maritima/source_rutas_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_ruta_importado(data){
    miurl = "/importacion_maritima/add_ruta_importado/";
...
function traer_embarques_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_maritima/source_embarque_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_embarque_importado(data){
    miurl = "/importacion_maritima/add_embarque_importado/";
...
function eliminar_agregado(master,id_house){
        ...
        $.ajax({
        type: "POST",
        url: "/importacion_maritima/source_embarque_id/",
        data: {
            id: id_house,
        ...
        success: function(response) {
            $.ajax({
                type: "POST",
                url: "/importacion_maritima/source_seguimiento_id/",
                data: {
...
function gastos_btn_h_click(){
        ...
        if ($.fn.dataTable.isDataTable('#table_edit_im')) {
            ...
            consignatario_code = $('#tabla_house_directo_im').DataTable().row('.table-secondary').data()[21];
        }
        if (selectedRowN!=null) {
            get_datos_gastos_house();
            $('#gastos_form_house').trigger("reset");
...
//                 $('#table_add_im tbody tr').removeClass('table-secondary');
//                $('#table_edit_im tbody tr').removeClass('table-secondary');
//                $('#tabla_house_directo_im_im tbody tr').removeClass('table-secondary');
                }
            })
...
function get_datos_seguimiento_rutas(numero) {
    $.ajax({
        url: "/importacion_maritima/datos_embarque_ruta/",  // Asegúrate de que esta URL coincida con tu Django URLConf
        type: "POST",
        data: {
            numero: numero,
...
function get_sugerencias_envases(numero) {
    ...
    $.ajax({
        type: "GET",
        url: "/importacion_maritima/get_sugerencias_envases/" + numero,
        success: function(response) {
        console.log(response.data);
 ...
 let numero=localStorage.getItem('num_house_gasto');
   ...
   $("#tabla_embarques_house").dataTable().fnDestroy();
    table_embarques = $('#tabla_embarques_house').DataTable({
        ...
        "ajax": {
            "url": "/importacion_maritima/source_embarques_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
$('.email').click(function () {
    ...
    $("#modalSeleccionEmailHouse4").dialog('close');
...
function cargar_notas(numero) {
    $('#notas_table').DataTable({
        ...
        ajax: {
            url: `/importacion_maritima/source/?numero=${numero}`,  // URL de la vista source
            dataSrc: 'data'
        },
        columns: [
            { data: 'id' },
...
function agregar_nota(event) {
    ...
    const url = "/importacion_maritima/guardar_notas/";
...
function eliminarNota(id) {
    if (confirm("¿Desea eliminar esta nota?")) {
        $.ajax({
            type: "POST",
            url: `/importacion_maritima/eliminar_nota/`,
            data: {
...
function abrir_modal_mails(e){
    ...
    $("#modalSeleccionEmailHouse4").dialog('open');
}
