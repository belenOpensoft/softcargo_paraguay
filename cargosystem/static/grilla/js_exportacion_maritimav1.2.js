...
//        }
        ...
        $("#add_master_modal").dialog({
            ...
            if ($('#table_add_em tbody tr').length === 0) {
...
var expandedRow;
    $('#tabla_expomarit tbody').on('click', 'td.details-control', function () {
        ...
        tr.removeClass('shown');
            ...
            $.ajax({
                url: '/exportacion_maritima/source_embarque_aereo/',
                type: 'POST',
                data: {
                    ...
                    buttons: [
                        {
                            ...
                            validarCoincidenciaAcumulados();
                                ...
                                $('#table_edit_em').DataTable().destroy();
    ...
    $('#tabla_expomarit tbody').on('dblclick', 'tr', function() {
        var tr = $(this).closest('tr');
        var row = table.row(tr);
        var rowData = row.data();
...
//    if (!$(event.target).closest('#table_edit_em').length) {
//        $('#table_edit_em tbody tr').removeClass('table-secondary');
//    }
//    //tabla de houses en add master
//    if (!$(event.target).closest('#table_add_em').length) {
//        $('#table_add_em tbody tr').removeClass('table-secondary');
//    }
});
   ...
   //form add house
                    ...
                    buttons: [
                        {
                           ...
                           click: function () {
                               $(this).dialog("close");
                               if (table_add_em instanceof $.fn.dataTable.Api) {
                               $('#table_add_em').DataTable().destroy();
                               $('#segment_response').css({'display':'none'});
                               }
                           },
                       },
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (table_add_em instanceof $.fn.dataTable.Api) {
                    table_add_em.ajax.reload(null, false);
                } else {
                    cargar_hauses_master();
                }
            }else if(lugar_editar==='editar'){
            if (table_edit_em instanceof $.fn.dataTable.Api) {
                    table_edit_em.ajax.reload(null, false);
                } else {
                    cargar_hauses_master_edit();
                }
                ...
                success: function (resultado) {
                    if (resultado['resultado'] === 'exito') {
                       ...
                       document.getElementById('numero_gasto_house').value=aux;
                        if ($.fn.DataTable.isDataTable('#table_add_em')) {
                            $('#table_add_em').DataTable().ajax.reload(null, false);
                        }

                        if ($.fn.DataTable.isDataTable('#table_edit_em')) {
                            $('#table_edit_em').DataTable().ajax.reload(null, false);
                        }
                        if ($.fn.DataTable.isDataTable('#tabla_house_directo_em')) {
                            $('#tabla_house_directo_em').DataTable().ajax.reload(null, false);
                        }
                    } else {
                        alert(resultado['resultado']);
...
table_add_em = $('#table_add_em').DataTable({
    "stateSave": true,
    "info":false,
    "dom": 'Btipr',
    "bAutoWidth": false,
    ...
    },
    {
        "targets": [7],
        "render": function (data, type, row, meta) {
            return row[29]; // Toma el índice 6 para la columna 7
        }
    },
    {
        "targets": [8],
        "render": function (data, type, row, meta) {
            return row[28]; // Toma el índice 7 para la columna 8
        }
    },
    {
        "targets": [9],
        "render": function (data, type, row, meta) {
            return row[27]; // Toma el índice 8 para la columna 9
        }
    },
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_add_em th').css({'width':'auto'});
         $('#table_add_em_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    $('#table_add_em tbody').off('dblclick').on('dblclick', 'tr', function () {
        var tr = $(this).closest('tr');
        var row = table_add_em.row(tr);
        var rowData = row.data();
    ...
    $('#table_add_em tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_add_em tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('tabla_origen', 'table_add_em');
...
//let master='MOLU13000250048';
let csrftoken = getCookie2('csrftoken');
table_edit_em = $('#table_edit_em').DataTable({
    "stateSave": true,
    "info":false,
    "dom": 'Btipr',
    "bAutoWidth": false,
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_edit_em th').css({'width':'auto'});
         $('#table_edit_em_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    //notas
            ...
            $('td:eq(1)', row).html(texto + " " + data[20]);
    ...
    $('#table_edit_em tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_edit_em tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('id_house_gasto', selectedRowId);
        localStorage.setItem('num_house_gasto', selectedRowN);
        localStorage.setItem('tabla_origen', 'table_edit_em');
...
function traer_notas_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/exportacion_maritima/source_notas_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_nota_importado(data) {
    const miurl = "/exportacion_maritima/add_nota_importado/";
                ...
                success: function (data) {
                    ...
                    if (localStorage.getItem('lugar_importarhijo') === 'editmaster') {
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_edit_em.rows('.table-secondary').data();
                            if (row.length === 1) {
                                let master = localStorage.getItem('master_editar');
                        ...
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_add_em.rows('.table-secondary').data();
                            if (row.length === 1) {
                                miurl = "/exportacion_maritima/eliminar_house/";
                                var toData = {
                                    'id': row[0][0],
         ...
         url='master-detail/';
            ...
            url: '/exportacion_maritima/'+url,
                ...
                success: function (data) {
                    ...
                    if ($.fn.dataTable.isDataTable('#table_edit_em')) {
                        consignatario_code = $('#table_edit_em').DataTable().row('.table-secondary').data()[19];
                    } else {
                        consignatario_code = $('#tabla_house_directo_em').DataTable().row('.table-secondary').data()[19];
                    }
                    if (selectedRowN != null) {
                        get_datos_gastos_house();
                        $('#gastos_form_house').trigger("reset");
...
function get_datos_seguimiento_rutas(numero) {
    $.ajax({
        url: "/exportacion_maritima/datos_embarque_ruta/",  // Asegúrate de que esta URL coincida con tu Django URLConf
        type: "POST",
        data: {
            numero: numero,
...
$('.email').click(function () {
        ...
        var row = $('#table_edit_em').DataTable().rows('.table-secondary').data();
        $("#id_to").val('');
        $("#id_cc").val('');
        $("#id_cco").val('');
        cco = $("#id_subject").val('');
...
let lugar=localStorage.getItem('lugar');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            table_edit_em.ajax.reload(false);
            }else if(lugar==='edit_directo'){
            $('#tabla_house_directo_em').DataTable().ajax.reload(false);
            }
            else{
            console.log('error en el lugar '+lugar);
            }
...
function acumulados(master, callback) {
    ...
    $.ajax({
        url: '/exportacion_maritima/source_embarque_aereo_full/' + master + '/',
        method: 'GET',
        success: function(response) {
...
function filtrar_tabla_master(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/exportacion_maritima/buscar_registros/',
         ...
         url='master-detail/';
            ...
            url: '/exportacion_maritima/'+url,
                ...
                success: function (data) {
                    ...
                    row = table_edit_em.rows('.table-secondary').data();
                    if (row.length === 1) {
                        $("#logs_modal").dialog({
                            autoOpen: true,
                            open: function () {
