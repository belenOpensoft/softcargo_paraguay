...
//});
       $(document).on("submit", "#searchFormDirecto", function(e) {
        ...
        open: function (event, ui) {
            ...
            $('#buscadorEmailsHouse11').val('');
            $("#listaEmailsHouse11 tr").show();
        }
    });
$('#buscadorEmailsHouse11').on('keyup', function () {
    let valor = $(this).val().toLowerCase();
    $("#listaEmailsHouse11 tr").filter(function () {
        $(this).toggle($(this).text().toLowerCase().indexOf(valor) > -1);
    });
});
    //buscadores
    $('#tabla_house_directo_et tfoot th').each(function(index) {
        let title = $('#tabla_house_directo_et th').eq(index).text();
    ...
    // Tabla general
    table = $('#tabla_house_directo_et').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/exportacion_terrestre/source_embarque_consolidado/",
            'type': 'GET',
            "data": function (d) {
                // Obtener los valores de búsqueda de los inputs del footer
                $('#tabla_house_directo_et tfoot input').each(function() {
    ...
    $('#tabla_house_directo_et tbody').off('dblclick').on('dblclick', 'tr', function () {
                    var table = $('#tabla_house_directo_et').DataTable();
    ...
    $('#tabla_house_directo_et tbody').off('click').on('click', 'tr', function (event) {
        ...
        localStorage.setItem('tabla_origen', 'tabla_house_directo_et');
...
if (!$('#id_awbhijo').val()) {
    ...
    //mails
        $('.email2').click(function () {
            $("#modalSeleccionEmailHouse11").dialog('close');
                ...
                let row = $('#tabla_house_directo_et').DataTable().rows('.table-secondary').data();
                $("#id_to").val('');
                $("#id_cc").val('');
                $("#id_cco").val('');
                cco = $("#id_subject").val('');
...
function abrir_modal_mails_d(e){
    ...
    $("#modalSeleccionEmailHouse11").dialog('open');
...
function filtrar_tabla_houses(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/exportacion_terrestre/buscar_registros_directos/',
