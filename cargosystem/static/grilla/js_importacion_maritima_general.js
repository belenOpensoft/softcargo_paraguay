...
$(document).ready(function () {
    ...
    table_general = $('#tabla_general_im').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/importacion_maritima/source_embarques_general",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
        ...
        initComplete: function () {
            ...
            let state = table_general.state.loaded();
            if (state) {
    ...
    table_general.columns().iterator('column', function (ctx, idx) {
        table_general.column(idx).nodes().to$().addClass('columna-acciones');
    });

    table_general.on('draw', function () {
        // Evento tras redibujado
    });

    $(table_general.table().container()).on('keyup', 'tfoot input', function () {
        $(this).addClass('is-invalid');
        table_general
    ...
    $('#tabla_general_im tfoot th').each(function(index) {
        let title = $('#tabla_general_im thead th').eq(index).text();
    ...
    $('#tabla_general_im tbody').on('click', 'tr', function (event) {
        event.stopPropagation();

        if (!$(this).hasClass('table-secondary')) {
            $('#tabla_general_im tbody tr').removeClass('table-secondary');
            $(this).addClass('table-secondary');
        }
    });

    $('#tabla_general_im tbody').on('dblclick', 'tr', function() {
        var tr = $(this).closest('tr');
        var row = table_general.row(tr);
        ...
        if (rowData) {
            ...
            localStorage.setItem('tabla_origen', 'tabla_general_im');
    ...
    alert('No se admiten valores negativos en los campos numéricos.')
        ...
        $('#edit_house_form_general').attr('action', '/importacion_maritima/edit_house/' + numero + '/');
...
function filtrar_tabla_general(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/importacion_maritima/buscar_registros_general/',
