...
$(document).ready(function () {
    ...
    /* DATATABLES */
    //buscadores
$('#tabla_importterrestre tfoot th').each(function(index) {
    let title = $('#tabla_importterrestre thead th').eq(index).text();
    ...
    table = $('#tabla_importterrestre').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/importacion_terrestre/source_master",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "buscar": buscar,
...
//        }
    ...
    $.ajax({
        type: "POST",
        url: "/importacion_terrestre/add_master/",
        data: formData,
        ...
        //ver mas
var expandedRow;
    $('#tabla_importterrestre tbody').on('click', 'td.details-control', function () {
     ...
     //modificar master
    $('#tabla_importterrestre tbody').on('click', 'td', function () {
    ...
    if (selectedRowId !== null) {
        $.ajax({
            url: '/importacion_terrestre/master-detail',
                    ...
                    buttons: [
                        {
                            ...
                            validarCoincidenciaAcumulados();
                                ...
                                $('#table_edit_it').DataTable().destroy();
    ...
    alert('No se admiten valores negativos.');
        ...
        $('#edit_master_form').attr('action', '/importacion_terrestre/edit_master/' + id_master + '/');
...
//                    $('#table_edit_it').DataTable().destroy();
   ...
   let vector = JSON.parse(localStorage.getItem('fecha_editada_house')) || [];
    ...
    // tabla general
    if (!$(event.target).closest('#tabla_importterrestre').length) {
        $('#tabla_importterrestre tbody tr').removeClass('table-secondary');
    }
//    //tabla de houses en edit master
//    if (!$(event.target).closest('#table_edit_it').length) {
//        $('#table_edit_it tbody tr').removeClass('table-secondary');
//    }
//    //tabla de houses en add master
//    if (!$(event.target).closest('#table_add_it').length) {
//        $('#table_add_it tbody tr').removeClass('table-secondary');
//    }
});
    $('#tabla_importterrestre tbody').on('click', 'tr', function (event) {
   ...
   //form add house
                    ...
                    buttons: [
                        {
                           ...
                           click: function () {
                               $(this).dialog("close");
                               if (table_add_it instanceof $.fn.dataTable.Api) {
                               $('#table_add_it').DataTable().destroy();
                               $('#segment_response').css({'display':'none'});
                               }
                           },
                       },
    ...
    $.ajax({
        type: "POST",
        url: "/importacion_terrestre/add_house/",
        data: formData,
        success: function(response) {
            if (response.success) {
            if(lugar_editar==='agregar'){
            if (table_add_it instanceof $.fn.dataTable.Api) {
                    table_add_it.ajax.reload(null, false);
                } else {
                    cargar_hauses_master();
                }
            }else if(lugar_editar==='editar'){
            if (table_edit_it instanceof $.fn.dataTable.Api) {
                    table_edit_it.ajax.reload(null, false);
                } else {
                    cargar_hauses_master_edit();
                }
   ...
   //modificar house
    ...
    alert('No se admiten valores negativos en los campos numéricos.')
        ...
        var formData = $(this).serialize();
        $('#edit_house_form').attr('action', '/importacion_terrestre/edit_house/' + numero + '/');
    ...
    if (confirm("¿Confirma guardar el gasto?")) {
        ...
        let numero=localStorage.getItem('numero_master_seleccionado');
            ...
            miurl = "/importacion_terrestre/add_gasto_master/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    if (confirm("¿Confirma guardar el gasto?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/importacion_terrestre/add_gasto_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
                ...
                success: function (resultado) {
                    if (resultado['resultado'] === 'exito') {
                       ...
                       document.getElementById('numero_gasto_house').value=aux;
                        if ($.fn.DataTable.isDataTable('#table_add_it')) {
                            $('#table_add_it').DataTable().ajax.reload(null, false);
                        }

                        if ($.fn.DataTable.isDataTable('#table_edit_it')) {
                            $('#table_edit_it').DataTable().ajax.reload(null, false);
                        }
                        if ($.fn.DataTable.isDataTable('#tabla_house_directo_it')) {
                            $('#tabla_house_directo_it').DataTable().ajax.reload(null, false);
                        }
                    } else {
                        alert(resultado['resultado']);
    ...
    if (confirm("¿Confirma guardar la ruta?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/importacion_terrestre/add_ruta_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    if (confirm("¿Confirma guardar el envase?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/importacion_terrestre/add_envase_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
    ...
    if (confirm("¿Confirma guardar el envase?")) {
        ...
        let numero=localStorage.getItem('num_house_gasto');
            ...
            let data = JSON.stringify(formData);
            miurl = "/importacion_terrestre/add_embarque_house/";
            var toData = {
                'numero':numero ,
                'data': data,
                'csrfmiddlewaretoken': csrf_token,
                    ...
                    },
                {
                    ...
                    click: function () {
                        if (confirm('¿Confirma descargar el archivo seleccionado?')) {
                            var url = '/importacion_terrestre/descargar_archivo/' + localStorage.getItem('id_archivo');  // Ruta de la vista que devuelve el archivo
                    ...
                    click: function () {
                        if (confirm('¿Confirma eliminar archivo?')) {
                            if (localStorage.getItem('id_archivo')) {
                                miurl = "/importacion_terrestre/eliminar_archivo/";
                                var toData = {
    ...
    $('#guardar_archivo').click(function () {
        if (confirm("¿Confirma guardar archivo?")) {
            ...
            miurl = "/importacion_terrestre/guardar_archivo/";
            $.ajax({
                type: "POST",
                url: miurl,
                data: formData,
...
table_add_it = $('#table_add_it').DataTable({
    "stateSave": true,
    "info":false,
    "dom": 'Btipr',
    "bAutoWidth": false,
    ...
    },
    {
        ...
        "className": 'derecha',
        "render": function (data, type, row, meta) {
            return row[20]; // Toma el índice 22 para la columna 4
        }
    },
    {
        "targets": [5],
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_add_it th').css({'width':'auto'});
         $('#table_add_it_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    //notas
            ...
            $('td:eq(3)', row).html(texto + " " + data[20]);

        },
    "initComplete": function() {
     //doble click modificar house en add_master form
    $('#table_add_it tbody').off('dblclick').on('dblclick', 'tr', function () {
        var tr = $(this).closest('tr');
        var row = table_add_it.row(tr);
        var rowData = row.data();
    ...
    $('#table_add_it tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_add_it tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('tabla_origen', 'table_add_it');
...
function getNameByIdVendedor(id) {
    var name = "";
    $.ajax({
        url: '/importacion_terrestre/get_name_by_id_vendedor',
        data: { id: id},
        async: false,
        success: function (response) {
            name = response.name;
...
var name = "";
$.ajax({
    url: '/importacion_terrestre/get_name_by_id',
    data: { id: id},
    async: false,
    success: function (response) {
        name = response.name;
...
//let master='MOLU13000250048';
let csrftoken = getCookie2('csrftoken');
table_edit_it = $('#table_edit_it').DataTable({
    "stateSave": true,
    "info":false,
    "dom": 'Btipr',
    "bAutoWidth": false,
    ...
    "ajax": {
        ...
        "dataSrc": function (json) {
         $('#table_edit_it th').css({'width':'auto'});
         $('#table_edit_it_wrapper .dataTables_scrollBody').css({
        'height': 'fit-content',
        });
    ...
    $('#table_edit_it tbody').off('dblclick').on('dblclick', 'tr', function () {
        let selectedRowN = localStorage.getItem('id_master_editar');
        let selected = this;
    ...
    $('#table_edit_it tbody').off('click').on('click', 'tr', function (event) {
    event.stopPropagation();
    if ($(this).hasClass('table-secondary')) {
    } else {
        $('#table_edit_it tbody tr').removeClass('table-secondary');
    ...
    if (rowData) {
        ...
        localStorage.setItem('id_house_gasto', selectedRowId);
        localStorage.setItem('num_house_gasto', selectedRowN);
        localStorage.setItem('tabla_origen', 'table_edit_it');
...
function traer_seguimientos() {
    ...
    $.ajax({
        url: '/importacion_terrestre/source_seguimientos_importado/',
        type: 'POST',
        dataType: 'json',
...
function traer_gastos_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_terrestre/source_gastos_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
...
function guardar_gasto_importado(data){
    miurl = "/importacion_terrestre/add_gasto_importado/";
...
function traer_envases_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_terrestre/source_envases_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_envase_importado(data){
    miurl = "/importacion_terrestre/add_envase_importado/";
...
function traer_rutas_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_terrestre/source_rutas_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_ruta_importado(data){
    miurl = "/importacion_terrestre/add_ruta_importado/";
...
function traer_embarques_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_terrestre/source_embarque_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_embarque_importado(data){
    miurl = "/importacion_terrestre/add_embarque_importado/";
...
function traer_archivos_importado(numeros, numeros_guardados){
    ...
    // Hacer la solicitud AJAX para enviar los IDs al servidor
     const csrftoken = getCookie2('csrftoken');
    $.ajax({
        url: '/importacion_terrestre/source_archivos_importado/',
        type: 'POST',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify({ ids: numeros }),
...
function guardar_archivo_importado(data) {
    const miurl = "/importacion_terrestre/add_archivo_importado/";
                ...
                success: function (data) {
                    ...
                    if (localStorage.getItem('lugar_importarhijo') === 'editmaster') {
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_edit_it.rows('.table-secondary').data();
                            if (row.length === 1) {
                                let master = localStorage.getItem('master_editar');
                        ...
                        if (confirm('¿Confirma eliminar seleccionado?')) {
                            let row = table_add_it.rows('.table-secondary').data();
                            if (row.length === 1) {
                                miurl = "/importacion_terrestre/eliminar_house/";
                                var toData = {
                                    'id': row[0][0],
...
function eliminar_agregado(master,id_house){
        ...
        $.ajax({
        type: "POST",
        url: "/importacion_terrestre/source_embarque_id/",
        data: {
            id: id_house,
        ...
        success: function(response) {
            $.ajax({
                type: "POST",
                url: "/importacion_terrestre/source_seguimiento_id/",
                data: {
...
let numero=localStorage.getItem('numero_master_seleccionado');
    ...
    table_gastos = $('#tabla_gastos').DataTable({
        ...
        "ajax": {
            "url": "/importacion_terrestre/source_gastos/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
function get_datos_gastos_house() {
    ...
    table_gastos = $('#tabla_gastos_house').DataTable({
        ...
        "ajax": {
            "url": "/importacion_terrestre/source_gastos_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
         ...
         url='master-detail/';
            ...
            url: '/importacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    if ($.fn.dataTable.isDataTable('#table_edit_it')) {
                        consignatario_code = $('#table_edit_it').DataTable().row('.table-secondary').data()[20];
                    } else {
                        consignatario_code = $('#tabla_house_directo_it').DataTable().row('.table-secondary').data()[20];
                    }
                    if (selectedRowN != null) {
                        get_datos_gastos_house();
                        $('#gastos_form_house').trigger("reset");
...
function get_datos_rutas_house() {
   ...
   $("#tabla_rutas_house").dataTable().fnDestroy();
    table_rutas = $('#tabla_rutas_house').DataTable({
        ...
        "ajax": {
            "url": "/importacion_terrestre/source_rutas_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
function get_datos_seguimiento_rutas(numero) {
    $.ajax({
        url: "/importacion_terrestre/datos_embarque_ruta/",  // Asegúrate de que esta URL coincida con tu Django URLConf
        type: "POST",
        data: {
            numero: numero,
...
function get_datos_envases_house(){
   ...
   $("#tabla_envases_house").dataTable().fnDestroy();
    table_envases = $('#tabla_envases_house').DataTable({
        ...
        "ajax": {
            "url": "/importacion_terrestre/source_envases_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
function get_sugerencias_envases(numero) {
    ...
    $.ajax({
        type: "GET",
        url: "/importacion_terrestre/get_sugerencias_envases/" + numero,
        success: function(response) {
        console.log(response.data);
 ...
 let numero=localStorage.getItem('num_house_gasto');
   ...
   $("#tabla_embarques_house").dataTable().fnDestroy();
    table_embarques = $('#tabla_embarques_house').DataTable({
        ...
        "ajax": {
            "url": "/importacion_terrestre/source_embarques_house/",
            'type': 'GET',
            "data": function (d) {
                return $.extend({}, d, {
                    "numero": numero,
...
$('.email').click(function () {
    ...
    $("#modalSeleccionEmailHouse9").dialog('close');
        ...
        var row = $('#table_edit_it').DataTable().rows('.table-secondary').data();
        $("#id_to").val('');
        $("#id_cc").val('');
        $("#id_cco").val('');
        cco = $("#id_subject").val('');
...
let lugar=localStorage.getItem('lugar');
    ...
    $.ajax({
        type: "POST",
        url: "/importacion_terrestre/modificar_fecha_retiro/",
...
function get_datos_pdf() {
    let selectedRowN = localStorage.getItem('num_house_gasto');
    miurl = "/importacion_terrestre/get_datos_caratula/";
    var toData = {
        'numero': selectedRowN,
        'csrfmiddlewaretoken': csrf_token,
    };
            ...
            url: '/importacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    let tabla_factura = $('#facturar_table').DataTable({
                        ...
                        "ajax": {
                            "url": "/importacion_terrestre/source_gastos_house_preventa/",
                            'type': 'GET',
                            "data": function (d) {
                                return $.extend({}, d, {
                                    "numero": numero,
...
function filtrar_tabla_master(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/importacion_terrestre/buscar_registros/',
         ...
         url='master-detail/';
            ...
            url: '/importacion_terrestre/'+url,
                ...
                success: function (data) {
                    ...
                    row = table_edit_it.rows('.table-secondary').data();
                    console.log(row);
                    if (row.length === 1) {
                        $("#logs_modal").dialog({
                            autoOpen: true,
