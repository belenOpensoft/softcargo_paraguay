...
//});
       $(document).on("submit", "#searchFormDirecto", function(e) {
        ...
        $("#modalSeleccionEmailHouse").dialog({
        autoOpen: false,
        modal: true,
        width: 400,
        height: 400,
        ...
        open: function (event, ui) {
            ...
            $('#buscadorEmailsHouse').val('');
            $("#listaEmailsHouse tr").show();
        }
    });
$('#buscadorEmailsHouse').on('keyup', function () {
    let valor = $(this).val().toLowerCase();
    $("#listaEmailsHouse tr").filter(function () {
        $(this).toggle($(this).text().toLowerCase().indexOf(valor) > -1);
    });
});

    //buscadores
    $('#tabla_house_directo_ia tfoot th').each(function(index) {
        let title = $('#tabla_house_directo_ia th').eq(index).text();
    ...
    // Tabla general
    table = $('#tabla_house_directo_ia').DataTable({
        "stateSave": true,
        "dom": 'Btlipr',
        "scrollX": true,
        "bAutoWidth": false,
        ...
        "pageLength": 100,
        "ajax": {
            "url": "/importacion_aerea/source_embarque_consolidado/",
            'type': 'GET',
            "data": function (d) {
                // Obtener los valores de búsqueda de los inputs del footer
                $('#tabla_house_directo_ia tfoot input').each(function() {
    ...
    $('#tabla_house_directo_ia tbody').off('dblclick').on('dblclick', 'tr', function () {
                    var table = $('#tabla_house_directo_ia').DataTable();
    ...
    $('#tabla_house_directo_ia tbody').off('click').on('click', 'tr', function (event) {
        ...
        localStorage.setItem('tabla_origen', 'tabla_house_directo_ia');
...
if (!$('#id_awbhijo').val()) {
    ...
    //mails
        $('.email2').click(function () {
            $("#modalSeleccionEmailHouse").dialog('close');
                ...
                var row = $('#tabla_house_directo_ia').DataTable().rows('.table-secondary').data();
                $("#id_to").val('');
                $("#id_cc").val('');
                $("#id_cco").val('');
                cco = $("#id_subject").val('');
...
function abrir_modal_mails_d(e){
    ...
    $("#modalSeleccionEmailHouse").dialog('open');
...
function filtrar_tabla_houses(data, e) {
    ...
    $.ajax({
        type: "POST",
        url: '/importacion_aerea/buscar_registros_directos/',
