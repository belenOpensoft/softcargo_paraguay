...
from consultas_administrativas.forms import ReporteCobranzasForm
...
def reporte_cobranzas(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return generar_excel_cobranzas(
                cobranzas,
                clientes_dict,
                asientos_dict,
                fecha_desde,
...
def generar_excel_cobranzas(cobranzas, clientes_dict, asientos_dict, fecha_desde, fecha_hasta,
    ...
    try:
        ...
        nombre_archivo = f"Reporte_Cobranzas_{fecha_desde}_al_{fecha_hasta}.xlsx"
        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {'in_memory': True})
        worksheet = workbook.add_worksheet("Cobranzas")
        ...
        # Título
        titulo = f'Cobranza entre el {fecha_desde.strftime("%d/%m/%Y")} y el {fecha_hasta.strftime("%d/%m/%Y")} en {nombre_moneda}'
        ...
        for cob in cobranzas:
            ...
            asiento_principal = next((a for a in asientos_dict.get(cob.mautogen, []) if getattr(a, 'imputacion', None) == 2), None)
