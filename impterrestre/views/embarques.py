...
from impterrestre.models import ImpterraCargaaerea
...
def source_embarques(request):
    if is_ajax(request):
        ...
        registros = ImpterraCargaaerea.objects.filter(numero=numero).order_by(*order)
        ...
        resultado['recordsTotal'] = ImpterraCargaaerea.objects.filter(numero=numero).count()
...
def guardar_embarques(request):
    ...
    try:
        ...
        if 'id_embarque' in data and data['id_embarque']:
            ...
            try:
                registro = ImpterraCargaaerea.objects.get(id=data['id_embarque'])
            except ImpterraCargaaerea.DoesNotExist:
    ...
    try:
        ...
        if isinstance(data, list):
            for envase_data in data:
                # Crear el registro del modelo ImpterraCargaaerea
                registro = ImpterraCargaaerea()

                # Obtener los campos disponibles del modelo
                campos = [f.name for f in ImpterraCargaaerea._meta.fields]
...
def get_sugerencias_envases(request, numero):
    try:
        carga = ImpterraCargaaerea.objects.filter(numero=numero).first()
...
def eliminar_embarque(request):
    ...
    try:
        id = request.POST['id']
        ImpterraCargaaerea.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
