...
import json
from impterrestre.models import ImpterraEmbarqueaereo, ImpterraCargaaerea, ImpterraConexaerea, ImpterraServiceaereo, \
    ImpterraEnvases
...
def generar_posicion(request):
    ...
    ultima_reserva = ImpterraEmbarqueaereo.objects.filter(fechaingreso__year=anio_actual).order_by('-numero').first()
...
def source_embarque_id(request):
    try:
        if request.method == 'POST':
            ...
            try:
                embarque = ImpterraEmbarqueaereo.objects.get(numero=id_embarque)
