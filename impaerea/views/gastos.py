...
from impaerea.models import ImportServireserva, VGastosMaster, VGastosHouse, ImportServiceaereo
import json
...
def add_gasto_master(request):
    ...
    try:
        ...
        if 'id_gasto_id' in form_data and form_data['id_gasto_id']:
            ...
            registro = ImportServireserva.objects.get(id=form_data['id_gasto_id'])
        else:
    ...
    except ImportServireserva.DoesNotExist:
...
def eliminar_gasto_master(request):
    ...
    try:
        id = request.POST['id']
        ImportServireserva.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
...
def add_gasto_house(request):
    ...
    try:
        ...
        if 'id_gasto_id_house' in form_data and form_data['id_gasto_id_house']:
            # Modificar el registro existente
            registro = ImportServiceaereo.objects.get(id=form_data['id_gasto_id_house'])

        else:
            # Crear un nuevo registro si no existe
            registro = ImportServiceaereo()
    ...
    except ImportServiceaereo.DoesNotExist:
        resultado['resultado'] = 'Registro no encontrado.'
    except IntegrityError:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
        ...
        if request.method == 'POST':
            ...
            if isinstance(data, list):
                ...
                for gasto_data in data:
                    # Crear la instancia de ImportServiceaereo para cada registro
                    registro = ImportServiceaereo()
...
def eliminar_gasto_house(request):
    ...
    try:
        id = request.POST['id']
        ImportServiceaereo.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
