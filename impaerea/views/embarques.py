...
from impaerea.models import ImportCargaaerea
...
def source_embarques(request):
    if is_ajax(request):
        ...
        registros = ImportCargaaerea.objects.filter(numero=numero).order_by(*order)
        ...
        resultado['recordsTotal'] = ImportCargaaerea.objects.filter(numero=numero).count()
...
def guardar_embarques(request):
    ...
    try:
        ...
        if 'id_embarque' in data and data['id_embarque']:
            ...
            try:
                registro = ImportCargaaerea.objects.get(id=data['id_embarque'])
            except ImportCargaaerea.DoesNotExist:
    ...
    try:
        ...
        if isinstance(data, list):
            for envase_data in data:
                # Crear el registro del modelo ImportCargaaerea
                registro = ImportCargaaerea()

                # Obtener los campos disponibles del modelo
                campos = [f.name for f in ImportCargaaerea._meta.fields]
...
def eliminar_embarque(request):
    ...
    try:
        id = request.POST['id']
        ImportCargaaerea.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
