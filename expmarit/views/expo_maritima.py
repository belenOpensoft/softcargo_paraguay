...
from expmarit.forms import add_im_form, add_form, add_house, edit_form, edit_house, gastosForm, gastosFormHouse, \
    rutasFormHouse, emailsForm, envasesFormHouse, embarquesFormHouse, NotasForm, edit_house_general
...
def master_expo_maritima(request):
    try:
        if request.user.has_perms(["expmarit.view_master",]):
            ...
            return render(request, 'expmarit/grilla_datos.html',{
                'form': add_form(),
...
def source_embarque_aereo_full(request, master):
    if is_ajax(request):
        ...
        try:
            embarques = ExpmaritEmbarqueaereo.objects.filter(awb=master)
            ...
            registros_cargaaerea = ExpmaritCargaaerea.objects.filter(numero__in=numeros)
...
def house_importacion_maritima(request):
    try:
        if request.user.has_perms(["expmarit.view_vembarqueaereo",]):
            ...
            return render(request, 'expmarit/grilla_datos_hd.html',{
                'form_house': add_house(),
                'form_edit_house': edit_house(),
                'opciones_busqueda': opciones_busqueda,
                'form_gastos': gastosForm(),
...
def get_data_embarque_aereo(registros_filtrados):
    try:
        ...
        for registro in registros_filtrados:
            ...
            registro_json.append('' if registro.agenteportuario is None else str(registro.agenteportuario))  # Estado

            archivos = ExpmaritAttachhijo.objects.filter(numero=registro.numero).count() #13
            embarques = ExpmaritCargaaerea.objects.filter(numero=registro.numero).count()
            envases = ExpmaritEnvases.objects.filter(numero=registro.numero).count()
            gastos = ExpmaritServiceaereo.objects.filter(numero=registro.numero).count()
            rutas = ExpmaritConexaerea.objects.filter(numero=registro.numero).count() #17
            notas=ExpmaritFaxes.objects.filter(numero=registro.numero).count()
            registro_json.append(archivos)
            registro_json.append(embarques)
            registro_json.append(envases)
            registro_json.append(gastos)
            ...
            registro_json.append('' if registro.etd is None else str(registro.etd)[:10])  #22
            ...
            registro_json.append('' if registro.agente is None else str(registro.agente))  # 27
            registro_json.append('' if registro.transportista is None else str(registro.transportista))  # 28
            registro_json.append('' if registro.embarcador is None else str(registro.embarcador))  # 29
    ...
    try:
        ...
        if isinstance(archivos_data, list):
            for envase_data in archivos_data:
                ...
                campos = [f.name for f in ExpmaritAttachhijo._meta.fields]
...
def source_archivos(request):
    if is_ajax(request):
        ...
        """FILTRO REGISTROS"""
        registros = ExpmaritAttachhijo.objects.filter(numero=numero).order_by(*order)
        """PREPARO DATOS"""
        resultado = {}
        ...
        resultado['length'] = length
        resultado['draw'] = request.GET['draw']
        resultado['recordsTotal'] = ExpmaritAttachhijo.objects.filter(numero=numero).count()
        resultado['recordsFiltered'] = str(registros.count())
        data_json = json.dumps(resultado)
    else:
        data_json = 'fail'
...
def eliminar_archivo(request):
    ...
    try:
        id = request.POST['id']
        att = ExpmaritAttachhijo.objects.get(id=id)
...
def modificar_fecha_retiro(request):
    if request.method == "POST":
        try:
            ...
            registros = ExpmaritEmbarqueaereo.objects.filter(awb=master)
...
@login_required(login_url='/')
def embarques_exportacion_maritima(request):
    try:

        if request.user.has_perms(["expmarit.view_master", ]):
            return render(request, 'expmarit/grilla_datos_general.html', {
                'form': add_form(),
                'form_house': add_house(),
                'form_search_master': add_im_form(),
                'form_edit_master': edit_form(),
...
def source_embarques_general(request):
    if is_ajax(request):
        ...
        # Generar mapa de reservas
        reservas = ExpmaritReservas.objects.all().values_list('awb', 'numero')

        mapa_reservas = {
            (awb or '').strip(): {'numero': numero_reserva}
...
def get_data_general(registros_filtrados,mapa_reservas):
    try:
        ...
        for registro in registros_filtrados:
            ...
            nro_reserva = reserva_data.get('numero', '')
            id_reserva = reserva_data.get('numero', '')
            ...
            archivos = ExpmaritAttachhijo.objects.filter(numero=registro.numero).count()
            embarques = ExpmaritCargaaerea.objects.filter(numero=registro.numero).count()
            envases = ExpmaritEnvases.objects.filter(numero=registro.numero).count()
            gastos = ExpmaritServiceaereo.objects.filter(numero=registro.numero).count()
            rutas = ExpmaritConexaerea.objects.filter(numero=registro.numero).count()
            notas = ExpmaritFaxes.objects.filter(numero=registro.numero).count()
