...
from expmarit.models import ExpmaritServireserva, VGastosMaster, VGastosHouse, ExpmaritServiceaereo
import json
...
def add_gasto_master(request):
    ...
    try:
        ...
        if 'id_gasto_id' in form_data and form_data['id_gasto_id']:
            ...
            registro = ExpmaritServireserva.objects.get(id=form_data['id_gasto_id'])
        else:
            # Crear un nuevo registro si no existe
            registro = ExpmaritServireserva()
    ...
    except ExpmaritServireserva.DoesNotExist:
...
def eliminar_gasto_master(request):
    ...
    try:
        id = request.POST['id']
        ExpmaritServireserva.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
...
def add_gasto_house(request):
    ...
    try:
        ...
        if 'id_gasto_id_house' in form_data and form_data['id_gasto_id_house']:
            # Modificar el registro existente
            registro = ExpmaritServiceaereo.objects.get(id=form_data['id_gasto_id_house'])

        else:
            # Crear un nuevo registro si no existe
            registro = ExpmaritServiceaereo()
    ...
    except ExpmaritServiceaereo.DoesNotExist:
        resultado['resultado'] = 'Registro no encontrado.'
    except IntegrityError:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
...
def eliminar_gasto_house(request):
    ...
    try:
        id = request.POST['id']
        ExpmaritServiceaereo.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
