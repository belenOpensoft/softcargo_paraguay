...
def generar_posicion(request):
    ...
    ultima_reserva = ExpmaritEmbarqueaereo.objects.filter(fechaingreso__year=anio_actual).order_by('-numero').first()
...
def eliminar_house(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            ExpmaritFaxes.objects.filter(numero=id).delete()
...
def source_embarque_id(request):
    try:
        if request.method == 'POST':
            ...
            try:
                embarque = ExpmaritEmbarqueaereo.objects.get(numero=id_embarque)
