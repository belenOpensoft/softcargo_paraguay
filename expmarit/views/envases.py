...
def source_envases(request):
    if is_ajax(request):
        ...
        """FILTRO REGISTROS"""
        registros = ExpmaritEnvases.objects.filter(numero=numero).order_by(*order)
        """PREPARO DATOS"""
        resultado = {}
        ...
        resultado['recordsTotal'] = ExpmaritEnvases.objects.filter(numero=numero).count()
    ...
    try:
        ...
        if isinstance(data, list):
            for envase_data in data:
                ...
                campos = [f.name for f in ExpmaritEnvases._meta.fields]
...
def eliminar_envase(request):
    ...
    try:
        id = request.POST['id']
        ExpmaritEnvases.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
