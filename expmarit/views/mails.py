    ...
    if is_ajax(request):
        try:
            ...
            if embarque.consignatario:
                email_consignatario = Clientes.objects.filter(codigo=embarque.consignatario).values_list('emailem',flat=True).first()
            email_notificar = None
            if notificar and notificar.notificar:
                email_notificar = Clientes.objects.filter(codigo=notificar.notificar).values_list('emailem',
                                                                                                  flat=True).first()
