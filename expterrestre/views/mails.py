...
from mantenimientos.models import Productos, Clientes, Servicios, Vapores
...
def get_data_email_op(request):
    ...
    if is_ajax(request):
        try:
            ...
            texto += f"<p style='font-family: Courier New, Courier, monospace; font-size: 12px;'>DEPARTAMENTO DE EXPORTACIÓN TERRESTRE,</p>"
...
def get_data_html(row_number, row, row2, row3, title, texto, resultado,seguimiento,gastos,embarque,master,gastos_boolean):
 ...
 \
        ...
        resultado['asunto'] = (
            ...
            f'HB/l: {row.hawb} - Ship: {row.embarcador} - Consig: {row.consignatario}'
