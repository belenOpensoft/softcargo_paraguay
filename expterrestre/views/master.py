...
def generar_posicion():
        ...
        ultima_reserva = ExpterraReservas.objects.filter(fechaingreso__year=anio_actual).order_by('-numero').first()

        if ultima_reserva and ultima_reserva.posicion:
            ultima_posicion = ultima_reserva.posicion
            match = re.match(rf"ET{mes_actual}-(\d+)-\d{{4}}", ultima_posicion)
        ...
        nueva_posicion = f"ET{mes_actual}-{nuevo_codigo}-{anio_actual}"
...
def master_detail(request):
    if request.method == 'GET':
        ...
        if master_id:
            try:
                master = ExpterraReservas.objects.get(numero=master_id)
