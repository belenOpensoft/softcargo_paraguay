...
def master_importacion_maritima(request):
    try:
        if request.user.has_perms(["expterrestre.view_master",]):
            opciones_busqueda = {
            ...
            return render(request, 'expoterrestre/grilla_datos.html',{
                'form': add_form(),
...
def house_importacion_maritima(request):
    try:
        if request.user.has_perms(["expterrestre.view_vembarqueaereo",]):
            opciones_busqueda = {
                'cliente__icontains': 'CLIENTE',
                'embarcador__icontains': 'EMBARCADOR',
                'consignatario__icontains': 'CONSIGNATARIO',
            ...
            return render(request, 'expoterrestre/grilla_datos_hd.html',{
                'form_house': add_house(),
                'form_edit_house': edit_house(),
                'opciones_busqueda': opciones_busqueda,
                'form_gastos': gastosForm(),
...
def source_embarque_aereo_full(request, master):
    if is_ajax(request):
        ...
        try:
            embarques = ExpterraEmbarqueaereo.objects.filter(awb=master)
            ...
            registros_cargaaerea = ExpterraCargaaerea.objects.filter(numero__in=numeros)
...
def get_data_embarque_aereo(registros_filtrados):
    try:
        ...
        for registro in registros_filtrados:
            ...
            archivos = ExpterraAttachhijo.objects.filter(numero=registro.numero).count() #14
            embarques = ExpterraCargaaerea.objects.filter(numero=registro.numero).count()
...
def source_embarque_consolidado(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        registros = VEmbarqueaereo.objects.all()
        ...
        total_records = VEmbarqueaereo.objects.all().count()
    ...
    try:
        ...
        if isinstance(archivos_data, list):
            for envase_data in archivos_data:
                ...
                campos = [f.name for f in ExpterraAttachhijo._meta.fields]
...
def source_archivos(request):
    if is_ajax(request):
        ...
        """FILTRO REGISTROS"""
        registros = ExpterraAttachhijo.objects.filter(numero=numero).order_by(*order)
        """PREPARO DATOS"""
        resultado = {}
        ...
        resultado['length'] = length
        resultado['draw'] = request.GET['draw']
        resultado['recordsTotal'] = ExpterraAttachhijo.objects.filter(numero=numero).count()
        resultado['recordsFiltered'] = str(registros.count())
        data_json = json.dumps(resultado)
    else:
        data_json = 'fail'
...
def eliminar_archivo(request):
    ...
    try:
        id = request.POST['id']
        att = ExpterraAttachhijo.objects.get(id=id)
...
def modificar_fecha_retiro(request):
    if request.method == "POST":
        try:
            ...
            registros = ExpterraEmbarqueaereo.objects.filter(awb=master)
