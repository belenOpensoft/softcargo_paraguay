...
def source_embarques(request):
    if is_ajax(request):
        ...
        registros = ExpterraCargaaerea.objects.filter(numero=numero).order_by(*order)
        ...
        resultado['recordsTotal'] = ExpterraCargaaerea.objects.filter(numero=numero).count()
...
def guardar_embarques(request):
    ...
    try:
        ...
        if 'id_embarque' in data and data['id_embarque']:
            ...
            try:
                registro = ExpterraCargaaerea.objects.get(id=data['id_embarque'])
            except ExpterraCargaaerea.DoesNotExist:
    ...
    try:
        ...
        if isinstance(data, list):
            for envase_data in data:
                ...
                campos = [f.name for f in ExpterraCargaaerea._meta.fields]
...
def eliminar_embarque(request):
    ...
    try:
        id = request.POST['id']
        ExpterraCargaaerea.objects.get(id=id).delete()
        resultado['resultado'] = 'exito'
    except IntegrityError as e:
        resultado['resultado'] = 'Error de integridad, intente nuevamente.'
    except Exception as e:
